...
void stadisticas()
{
        conn = mysql_init(NULL);
        servidor();
        if (mysql_query(conn, "select V_dia from Venta_total where fecha = curdate()"))
        {
        }
        res = mysql_use_result(conn);
        while ((row = mysql_fetch_row(res)) != NULL)gtk_label_set_text(lbl_ventadia, row[0]);

        if (mysql_query(conn, "select count(Id_ticket) from Ticket where date(Fecha_hora) = curdate()"))
        {
        }
        res = mysql_use_result(conn);
        while ((row = mysql_fetch_row(res)) != NULL)gtk_label_set_text(lbl_clientes_dia, row[0]);

        if (mysql_query(conn, "select sum(Total) from Ticket where month(Fecha_hora) = month(curdate())"))
        {
        }
        res = mysql_use_result(conn);
        while ((row = mysql_fetch_row(res)) != NULL)gtk_label_set_text(lbl_ventames, row[0]);

        if (mysql_query(conn, "select sum(Total) from Ticket where year(Fecha_hora) = year(curdate())"))
        {
        }
        res = mysql_use_result(conn);
        while ((row = mysql_fetch_row(res)) != NULL)gtk_label_set_text(lbl_ventaanio, row[0]);

        if (mysql_query(conn, "select min(Total) from Ticket where year(Fecha_hora) = year(curdate())"))
        {
        }
        res = mysql_use_result(conn);
        while ((row = mysql_fetch_row(res)) != NULL)gtk_label_set_text(lbl_ventaanio_min, row[0]);

        if (mysql_query(conn, "select min(Total) from Ticket where month(Fecha_hora) = month(curdate())"))
        {
        }
        res = mysql_use_result(conn);
        while ((row = mysql_fetch_row(res)) != NULL)gtk_label_set_text(lbl_ventames_min, row[0]);

        if (mysql_query(conn, "select max(Total) from Ticket where year(Fecha_hora) = year(curdate())"))
        {
        }
        res = mysql_use_result(conn);
        while ((row = mysql_fetch_row(res)) != NULL)gtk_label_set_text(lbl_ventaanio_max, row[0]);

        if (mysql_query(conn, "select max(Total) from Ticket where month(Fecha_hora) = month(curdate())"))
        {
        }
        res = mysql_use_result(conn);
        while ((row = mysql_fetch_row(res)) != NULL)gtk_label_set_text(lbl_ventames_max, row[0]);

        if (mysql_query(conn, "select sum(Total) from Ticket where year(Fecha_hora) = month(curdate())-1"))
        {
        }
        res = mysql_use_result(conn);
        while ((row = mysql_fetch_row(res)) != NULL)gtk_label_set_text(lbl_anio_ant, row[0]);
...
void on_btn_Sesion_clicked()
{
        ...
        gtk_popover_set_modal(GTK_POPOVER(popovermenu1), 1);
}
...
void cierra_wz()
{
        gtk_widget_hide_on_delete(window_z);
}
...
void on_cancela_and_factura2_clicked()
{
        ...
        gtk_widget_set_sensitive(GTK_WIDGET(ety_cbarra),TRUE);
}
...
{
        gtk_popover_set_modal(GTK_POPOVER(popovermenu1), 0);
        gtk_entry_set_text(g_Entry_Usuario, "");
        gtk_entry_set_text(g_Entry_Contrasena, "");
...
void on_emp_aceptar_anadir_clicked()
{
        ...
        gtk_widget_set_sensitive(GTK_WIDGET(ety_produ_emp),TRUE);
...
{ // gtk_tree_selection_unselect_all(select3);
        ...
        sprintf(busqueda_fac, "select SKU , Nombre , Venta from Producto where Nombre LIKE '%%%s%%' or SKU ='%s' limit 30", consulta, consulta);
...
void on_btn_consulta_emp_clicked()
{
        ...
        while ((row = mysql_fetch_row(res)) != NULL)
        {
                ...
                gtk_widget_set_sensitive(GTK_WIDGET(ety_produ_emp),FALSE);
        }
...
void consulta_producto()
{
        ...
        while ((row = mysql_fetch_row(res)) != NULL)
        {
                ...
                gtk_widget_set_sensitive(GTK_WIDGET(ety_cbarra),FALSE);
        }
...
void consulta_xz()
{
        gtk_combo_box_text_remove_all(cb_user_arqueo);
        gtk_combo_box_text_remove_all(cb_user_corte);
        conn = mysql_init(NULL);
        conf();
        if (mysql_query(conn, "select User from Venta_ini"))
        {
                char tempErr[128];
                sprintf(tempErr, "%s", mysql_error(conn));
                gtk_label_set_text(lbl_error, tempErr);
        ...
        while ((row = mysql_fetch_row(res)) != NULL)
        {
                gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(cb_user_arqueo), row[0]);
                gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(cb_user_corte), row[0]);
        }
        mysql_free_result(res);
        mysql_close(conn);
}
...
void cierra_contrasena(){
        gtk_entry_set_text(ety_con_act,"");
        gtk_entry_set_text(ety_con_new,"");
        gtk_entry_set_text(ety_con_new2,"");
        gtk_widget_hide_on_delete(window_password);
}
void comprueba_contrasena(){
        const char *con_ant = gtk_entry_get_text(ety_con_act);
        const char *con_new = gtk_entry_get_text(ety_con_new);
        const char *contrasena = gtk_entry_get_text(g_Entry_Contrasena);
        const char *con_new2 = gtk_entry_get_text(ety_con_new2);

        if (strcmp(con_new, con_new2) == 0 && strcmp(con_new2, "") != 0 &&  strcmp(con_ant, contrasena) == 0)
                {
                        gtk_widget_set_sensitive(GTK_WIDGET(btn_cambiar_cont), TRUE);
                        gtk_revealer_set_reveal_child(GTK_REVEALER(info_bar_contra), FALSE);
                }
                else
                {
                        gtk_label_set_text(lbl_info_contra, "Las Contrasena no coinciden o estan vacias");
                        gtk_revealer_set_reveal_child(GTK_REVEALER(info_bar_contra), TRUE);
                        gtk_widget_set_sensitive(GTK_WIDGET(btn_cambiar_cont), FALSE);
                }
}
void cambia_contrasena(){
        const char *persona = gtk_entry_get_text(g_Entry_Usuario);
        const char *con_new2 = gtk_entry_get_text(ety_con_new2);
        char consu[200];
        conn = mysql_init(NULL);
        conf();
        sprintf(consu,"ALTER USER '%s'@'localhost' IDENTIFIED BY '%s'",persona,con_new2);
        ...
        mysql_close(conn);
        cierra_contrasena();
        on_btn_cambiar_usuario_clicked();
}
void abre_contrasena(){
        gtk_widget_show(window_password);
}
...
void copia_articulo()
{
        ...
        gtk_entry_set_text(ety_cbarra,"");
        gtk_widget_hide(pop_generar_sku);
        gtk_widget_grab_focus(GTK_WIDGET(ety_cbarra));
        gtk_widget_hide(pop_generar_sku);
}
...
void anadir_productocarrito()
{
        ...
        {
                sprintf(anadir_pro, "insert into Carrito_compra(No,Producto,P_unitario,Subtotal,SKU) select  1,Nombre,Venta,(Venta*1),SKU from Producto where SKU = %s ON DUPLICATE KEY UPDATE No=No+1, Subtotal=Venta*No", sku);
                conn = mysql_init(NULL);
                servidor();
...
void elimina_ticket()
{
        char *total = gtk_label_get_text(lbl_tickettotal);
        char consu[200];
        char consu2[200];
        char consu3[200];
        char consu4[200];
        ...
        sprintf(consu, "select Query,Usuario from Ticket where Id_ticket = %s", id);
        sprintf(consu3, "delete from Ticket where Id_ticket = %s", id);
        sprintf(consu6, "update Venta_total set V_dia=V_dia-%s where fecha = curdate()", total);
        conn = mysql_init(NULL);
        servidor();
        if (mysql_query(conn, consu))
        {
        ...
        while ((row = mysql_fetch_row(res)) != NULL)
        {
                ...
                sprintf(consu5, "update Venta_ini set V_dia=V_dia-%s where User = '%s'", total, row[1]);
        }
        mysql_free_result(res);
        mysql_close(conn);
        //===============================================================================================================================
...
void ingresa_venta()
{
        ...
        char anadir_pro4[200];
        char anadir_pro5[200];
        const char *Usuario = gtk_entry_get_text(g_Entry_Usuario);
        const char *recibido = gtk_entry_get_text(ety_recibido);
        const char *total = gtk_label_get_text(lbl_totalpos);
        const char *cambio = gtk_label_get_text(ety_cambio);
        const char *usuariolbl = gtk_label_get_text(lbl_usuario);
        ...
        else
        {
                ...
                sprintf(anadir_pro4, "insert into Venta_ini values(%s,'%s')ON DUPLICATE KEY UPDATE V_dia=V_dia+%s", total, usuariolbl, total);
                sprintf(anadir_pro2, "insert into Ticket (Usuario,Total,Recibido,Cambio,Query,Fecha_hora) values ('%s','%s','%s','%s','%s',NOW())", Usuario, total, recibido, cambio, dt_format);
                sprintf(anadir_pro3, "update Producto inner join `%s` on Producto.SKU = `%s`.SKU set Producto.Piezas=Producto.Piezas-`%s`.No;", dt_format, dt_format, dt_format);
                sprintf(anadir_pro5, "insert into Venta_total values(%s,curdate()) ON DUPLICATE KEY UPDATE V_dia=V_dia+%s", total, total);
                ...
                if (mysql_query(conn, anadir_pro4))
                {
                        char tempErr[128];
                        sprintf(tempErr, "%s", mysql_error(conn));
                        gtk_label_set_text(lbl_error, tempErr);
                ...
                if (mysql_query(conn, anadir_pro5))
                {
                        char tempErr[128];
                        sprintf(tempErr, "%s", mysql_error(conn));
                        gtk_label_set_text(lbl_error, tempErr);
...
void pagar_faltante()
{
        char consu[200];
        gchar *id;
        gtk_tree_model_get(model_corte, &iter_corte, colidz, &id, -1);
        sprintf(consu, "update Corte_z set Status='Pagado', F_pago=NOW() where Id = %s", id);
        conn = mysql_init(NULL);
        servidor();
        if (mysql_query(conn, consu))
        {
...
void filtra_faltante()
{
        if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(chk_faltante)))
        {
                conn = mysql_init(NULL);
                servidor();
                if (mysql_query(conn, "select * from Corte_z where Status='Faltante'"))
                {
                        char tempErr[128];
                        sprintf(tempErr, "%s", mysql_error(conn));
                        gtk_label_set_text(lbl_error, tempErr);
...
void selection_corte(gpointer data)
{
        char consu[200];
        gchar *id;
        if (gtk_tree_selection_get_selected(select6, &model_corte, &iter_corte))
        {
                gtk_tree_model_get(model_corte, &iter_corte, colidz, &id, -1);
                sprintf(consu, "select Status from Corte_z where Id = %s", id);
                conn = mysql_init(NULL);
                servidor();
                if (mysql_query(conn, consu))
                {
                ...
                res = mysql_use_result(conn);
                while ((row = mysql_fetch_row(res)) != NULL)
                        if (strcmp(row[0], "Faltante") == 0)
                        {
                                gtk_widget_set_sensitive(GTK_WIDGET(btn_pagar), 1);
                        }
                        else
                        {
                                gtk_widget_set_sensitive(GTK_WIDGET(btn_pagar), 0);
                        }
                mysql_free_result(res);
                mysql_close(conn);
        }
...
void selection_buscar(gpointer data)
{
        ...
        {
                gtk_tree_model_get(model_busc, &iter_busqueda, COLsku_bus, &holi, -1);
                sprintf(consu, "insert into Carrito_compra(No,Producto,P_unitario,Subtotal,SKU) select  1,Nombre,Venta,(Venta*1),SKU from Producto where SKU = %s ON DUPLICATE KEY UPDATE No=No+1, Subtotal=Venta*No", holi);
                conn = mysql_init(NULL);
                servidor();
                if (mysql_query(conn, consu))
                {
...
void cierra_advertenciaz()
{
        gtk_widget_hide_on_delete(advertencia_cortez);
}
...
void anade_vp()
{
        ...
        if (!strcmp(sku, "") == 0)
        {
                ...
                char consu[200];
                sprintf(consu, "insert into Carrito_compra values(%s,'%s',%s,%s,%s) ON DUPLICATE KEY UPDATE No=No+%s, Subtotal=%s*No", Cantidad, Articulo, P_articulo, Precio, sku, Cantidad, P_articulo);
                conn = mysql_init(NULL);
                servidor();
                if (mysql_query(conn, consu))
                {
...
void muestra_arqueo()
{
        gtk_widget_show(window_x);
        consulta_xz();
}
void oculta_arqueo()
{
        gtk_widget_hide_on_delete(window_x);
}
void prs_arqueo()
{
        const char *persona = gtk_combo_box_text_get_active_text(cb_user_arqueo);
        char user2[200];
        sprintf(user2, "%s", persona);
        if (strcmp(user2, "(null)") == 0)
        {
                gtk_label_set_text(lbl_error, "Seleccione la persona a hacer el arqueo");
                return gtk_widget_show(dialog_error_datos);
        }
        else
        {
                const char *dinero = gtk_entry_get_text(spin_cx);
                char consu[200];
                char con[99];
                double x, y, z;
                sprintf(consu, "select V_dia from Venta_ini where User='%s'", persona);
                conn = mysql_init(NULL);
                servidor();
                if (mysql_query(conn, consu))
                {
                ...
                if (z < 0)
                {
                        gtk_label_set_text(lbl_statusx, "Faltante");
                }
                else
                {
                        gtk_label_set_text(lbl_statusx, "Sobrante");
                }
                if (z == 0)
                {
                        gtk_label_set_text(lbl_statusx, "Tablas");
                }

                sprintf(con, "%.2f", z);
                gtk_revealer_set_reveal_child(GTK_REVEALER(reveal_x), TRUE);
                gtk_label_set_text(lbl_arqueo, con);
                mysql_free_result(res);
                mysql_close(conn);
        }
}
...
void prs_corte()
{
        const char *persona = gtk_combo_box_text_get_active_text(cb_user_corte);
        char user2[200];
        sprintf(user2, "%s", persona);
        if (strcmp(user2, "(null)") == 0)
        {
                gtk_label_set_text(lbl_error, "Seleccione la persona a hacer el corte");
                return gtk_widget_show(dialog_error_datos);
        }
        else
        {
                gtk_label_set_text(lbl_usuariocz, persona);
                gtk_widget_show(advertencia_cortez);
        }
}
void ingresa_cortez()
{
        const char *gerente = gtk_entry_get_text(g_Entry_Usuario);
        const char *persona = gtk_combo_box_text_get_active_text(cb_user_corte);
        const char *dinero = gtk_entry_get_text(spin_cz);
        const char *persona_corte = gtk_entry_get_text(spin_cz);
        char consu[200];
        char consu2[200];
        char consu_Z[900];
        char con[99];
        double x, y, z;
        sprintf(consu, "select V_dia from Venta_ini where User='%s'", persona);
        sprintf(consu2, "update Venta_ini set V_dia=0 where User='%s'", persona);
        conn = mysql_init(NULL);
        servidor();
        if (mysql_query(conn, consu))
        {
        ...
        if (z < 0)
        {
                gtk_label_set_text(lbl_statusz, "Faltante");
                sprintf(consu_Z, "insert into Corte_z (Gerente,Cajero,Ingreso,Balance,Status,Fecha) "
                                 "values ('%s','%s',%s,%.2f,'Faltante',NOW())",
                        gerente, persona, dinero, z);
        }
        else
        {
                gtk_label_set_text(lbl_statusz, "Sobrante");
                sprintf(consu_Z, "insert into Corte_z (Gerente,Cajero,Ingreso,Balance,Status,Fecha) "
                                 "values ('%s','%s',%s,%.2f,'Sobrante',NOW())",
                        gerente, persona, dinero, z);
        }
        if (z == 0)
        {
                gtk_label_set_text(lbl_statusz, "Tablas");
                sprintf(consu_Z, "insert into Corte_z (Gerente,Cajero,Ingreso,Balance,Status,Fecha) "
                                 "values ('%s','%s',%s,%.2f,'Tablas',NOW())",
                        gerente, persona, dinero, z);
        }
        if (mysql_query(conn, consu_Z))
        {
                char tempErr[128];
                sprintf(tempErr, "%s", mysql_error(conn));
                gtk_label_set_text(lbl_error, tempErr);
        ...
        sprintf(con, "%.2f", z);
        gtk_revealer_set_reveal_child(GTK_REVEALER(reveal_z), TRUE);
        gtk_label_set_text(lbl_corte, con);
        mysql_free_result(res);
        mysql_close(conn);
        gtk_widget_hide_on_delete(advertencia_cortez);
        refresca_corte();
