	...
	gpointer data)
{
	...
	gtk_list_store_set(GTK_LIST_STORE(model_inventario), &iter_inventario, COLinveingreso, "h", COLinveajuste, "h", -1);

	gtk_widget_show(g_Dialog_Error);
}
*/
static GtkTreeModel *reporte(void)
{
	store_reporte = gtk_list_store_new(columna_reporte, G_TYPE_STRING, G_TYPE_STRING);

	gtk_list_store_append(store_reporte, &iter_reporte);
	gtk_list_store_set(store_reporte, &iter_reporte,
					   cantidad_reporte, row[0],
					   fecha_reporte, row[1],-1);
	...
	return GTK_TREE_MODEL(store_reporte);
}

static GtkWidget *titulo_reporte(void)
{
	renderer_reporte = gtk_cell_renderer_text_new();
	column_repor = gtk_tree_view_column_new_with_attributes("Venta", renderer_reporte, "text", cantidad_reporte, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view_reporte), column_repor);
	renderer_reporte = gtk_cell_renderer_text_new();
	column_repor2 = gtk_tree_view_column_new_with_attributes("Fecha", renderer_reporte, "text", fecha_reporte, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view_reporte), column_repor2);
	

	model_reporte = reporte();
	gtk_tree_view_set_model(GTK_TREE_VIEW(view_reporte), model_reporte);
	g_object_unref(model_reporte);

	return view_reporte;
}


static GtkTreeModel *corte(void)
{
	store_corte = gtk_list_store_new(N_COLUMNS_corte, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, 
	G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING);

	gtk_list_store_append(store_corte, &iter_corte);
	gtk_list_store_set(store_corte, &iter_corte,
					   colidz, row[0],
					   colgerente, row[1],
					   colcajero, row[2],
					   colingresoz, row[3],
	...
	return GTK_TREE_MODEL(store_corte);
}

static GtkWidget *titulo_corte(void)
{
	renderer_corte = gtk_cell_renderer_text_new();
	column_corte = gtk_tree_view_column_new_with_attributes("ID", renderer_corte, "text", colidz, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view_corte), column_corte);
	renderer_corte = gtk_cell_renderer_text_new();
	column_corte2 = gtk_tree_view_column_new_with_attributes("Gerente", renderer_corte, "text", colgerente, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view_corte), column_corte2);
	renderer_corte = gtk_cell_renderer_text_new();
	column_corte3 = gtk_tree_view_column_new_with_attributes("Usuario", renderer_corte, "text", colcajero, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view_corte), column_corte3);
	renderer_corte = gtk_cell_renderer_text_new();
	column_corte4 = gtk_tree_view_column_new_with_attributes("Ingreso", renderer_corte, "text", colingresoz, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view_corte), column_corte4);
	renderer_corte = gtk_cell_renderer_text_new();
	column_corte5 = gtk_tree_view_column_new_with_attributes("Balance", renderer_corte, "text", colbalancez, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view_corte), column_corte5);
	renderer_corte = gtk_cell_renderer_text_new();
	column_corte6 = gtk_tree_view_column_new_with_attributes("Status", renderer_corte, "text", colstatusz, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view_corte), column_corte6);
	renderer_corte = gtk_cell_renderer_text_new();
	column_corte7 = gtk_tree_view_column_new_with_attributes("Fecha", renderer_corte, "text", colfechaz, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view_corte), column_corte7);
	renderer_corte = gtk_cell_renderer_text_new();
	column_corte8 = gtk_tree_view_column_new_with_attributes("Fecha Pago", renderer_corte, "text", colfechapz, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view_corte), column_corte8);

	model_corte = corte();
	gtk_tree_view_set_model(GTK_TREE_VIEW(view_corte), model_corte);
	g_object_unref(model_corte);
...
static GtkTreeModel *inve(void)
{
	...
	gtk_list_store_set(store_inve, &iter_inventario,
					   ...
					   COLinvepiezas, row[2],
					   COLinvevalor, row[3],
					   COLinveingreso, row[4],
					   COLfisico, row[5],
					   COLinveajuste, row[6], -1);
	while ((row = mysql_fetch_row(res)) != NULL)
	{
		gtk_list_store_append(store_inve, &iter_inventario);
		gtk_list_store_set(store_inve, &iter_inventario,
...
static GtkWidget *titulo_inventario(void)
{
	...
	renderer_inventario = gtk_cell_renderer_text_new();
	column_inve5 = gtk_tree_view_column_new_with_attributes("Fisico", renderer_inventario, "text", COLfisico, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view_inventario), column_inve5);
	renderer_inventario = gtk_cell_renderer_text_new();
	column_inve6 = gtk_tree_view_column_new_with_attributes("Balance", renderer_inventario, "text", COLinveajuste, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view_inventario), column_inve6);
...
static GtkTreeModel *inve_desc(void)
{
	store_inve_desc = gtk_list_store_new(N_COLUMNS_inve_des, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING);

	gtk_list_store_append(store_inve_desc, &iter_inventario_desc);
	gtk_list_store_set(store_inve_desc, &iter_inventario_desc,
					   COLinvesku_des, row[0],
					   COLinvenombre_des, row[1],
					   COLinvepiezas_des, row[2],
					   COLinvevalor_des, row[3],
					   COLinveingreso_des, row[4],
					   COLfisico_des, row[5],
					   COLinveajuste_des, row[6], -1);
	while ((row = mysql_fetch_row(res)) != NULL)
	{
		gtk_list_store_append(store_inve_desc, &iter_inventario_desc);
		gtk_list_store_set(store_inve_desc, &iter_inventario_desc,
	...
	return GTK_TREE_MODEL(store_inve_desc);
	// g_signal_connect (renderer_inventario, "edited", G_CALLBACK (celdas_editables), model_inventario);
}

static GtkWidget *titulo_inventario_desc(void)
{
	renderer_inventario_desc = gtk_cell_renderer_text_new();
	column_inve_desc0 = gtk_tree_view_column_new_with_attributes("SKU", renderer_inventario_desc, "text", COLinvesku_des, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view_descinve), column_inve_desc0);
	renderer_inventario_desc = gtk_cell_renderer_text_new();
	column_inve_desc = gtk_tree_view_column_new_with_attributes("Nombre", renderer_inventario_desc, "text", COLinvenombre_des, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view_descinve), column_inve_desc);
	renderer_inventario_desc = gtk_cell_renderer_text_new();
	column_inve_desc2 = gtk_tree_view_column_new_with_attributes("Piezas", renderer_inventario_desc, "text", COLinvepiezas_des, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view_descinve), column_inve_desc2);
	renderer_inventario_desc = gtk_cell_renderer_text_new();
	column_inve_desc3 = gtk_tree_view_column_new_with_attributes("Valor", renderer_inventario_desc, "text", COLinvevalor_des, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view_descinve), column_inve_desc3);
	renderer_inventario_desc = gtk_cell_renderer_text_new();
	column_inve_desc4 = gtk_tree_view_column_new_with_attributes("Ingreso", renderer_inventario_desc, "text", COLinveingreso_des, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view_descinve), column_inve_desc4);
	renderer_inventario_desc = gtk_cell_renderer_text_new();
	column_inve_desc5 = gtk_tree_view_column_new_with_attributes("Fisico", renderer_inventario_desc, "text", COLfisico_des, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view_descinve), column_inve_desc5);
	renderer_inventario_desc = gtk_cell_renderer_text_new();
	column_inve_desc6 = gtk_tree_view_column_new_with_attributes("Balance", renderer_inventario_desc, "text", COLinveajuste_des, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view_descinve), column_inve_desc6);

	model_inventario_desc = inve_desc();
	gtk_tree_view_set_model(GTK_TREE_VIEW(view_descinve), model_inventario_desc);
	g_object_unref(model_inventario_desc);

	return view_descinve;
}

static GtkTreeModel *inve_alt(void)
{
	store_inve_alt = gtk_list_store_new(NUM_COLS_invetario, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING);

	gtk_list_store_append(store_inve_alt, &iter_inventario_alt);
	gtk_list_store_set(store_inve_alt, &iter_inventario_alt,
					   COLcan_inve, row[0],
					   COLbalancef, row[1],
					   COLbalance, row[2],
					   COLusuario_inve, row[3],
					   COLfecha, row[4],
					   colquery, row[5], -1);
	while ((row = mysql_fetch_row(res)) != NULL)
	{
	...
	return GTK_TREE_MODEL(store_inve_alt);
	// g_signal_connect (renderer_inventario, "edited", G_CALLBACK (celdas_editables), model_inventario);
}

static GtkWidget *titulo_inventario_alt(void)
{
	renderer_inventario_alt = gtk_cell_renderer_text_new();
	column_inve_alt = gtk_tree_view_column_new_with_attributes("Cantidad", renderer_inventario_alt, "text", COLcan_inve, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view_inventario_alt), column_inve_alt);
	renderer_inventario_alt = gtk_cell_renderer_text_new();
	column_inve_alt2 = gtk_tree_view_column_new_with_attributes("Balance Fisico", renderer_inventario_alt, "text", COLbalancef, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view_inventario_alt), column_inve_alt2);
	renderer_inventario_alt = gtk_cell_renderer_text_new();
	column_inve_alt3 = gtk_tree_view_column_new_with_attributes("Balance Logico", renderer_inventario_alt, "text", COLbalance, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view_inventario_alt), column_inve_alt3);
	renderer_inventario_alt = gtk_cell_renderer_text_new();
	column_inve_alt4 = gtk_tree_view_column_new_with_attributes("Usuario", renderer_inventario_alt, "text", COLusuario_inve, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view_inventario_alt), column_inve_alt4);
	renderer_inventario_alt = gtk_cell_renderer_text_new();
	column_inve_alt5 = gtk_tree_view_column_new_with_attributes("Fecha", renderer_inventario_alt, "text", COLfecha, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view_inventario_alt), column_inve_alt5);
	renderer_inventario_alt = gtk_cell_renderer_text_new();

	model_inventario_alt = inve_alt();
	gtk_tree_view_set_model(GTK_TREE_VIEW(view_inventario_alt), model_inventario_alt);
	g_object_unref(model_inventario_alt);

	return view_inventario_alt;
}
...
static GtkTreeModel *bus(void)
{
	...
	store_busqueda = gtk_list_store_new(N_COLUMNS_BUS, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, GDK_TYPE_PIXBUF);

	gtk_list_store_append(store_busqueda, &iter_busqueda);
	gtk_list_store_set(store_busqueda, &iter_busqueda,
					   COLsku_bus, row[0],
					   COLproducto_pos, row[1],
					   COLventa_pos, row[2],
...
static GtkTreeModel *venta_ticket(void)
{
	...
	gtk_list_store_set(store_venta, &iter_venta,
					   producto_venta, row[1],
					   punitario_venta, row[2],
					   nop_venta, row[0],
